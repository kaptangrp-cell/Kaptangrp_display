...
const empty: Draft = {
  image_url: "", category: "BAGS", price: 0, in_stock: true, quantity: 1,
  size: "", title_en: "", title_de: "", description_en: "", description_de: "",
  material_en: "", material_de: "", color_en: "", color_de: "",
  notes_en: "", notes_de: "", sort_order: 0,
};
...
function AdminPage() {
  ...
  const q = useQuery({ ...productsQuery(), enabled: checked });
  const [editing, setEditing] = useState<Draft | null>(null);

  if (!checked) return <div className="pt-32 text-center text-muted-foreground">…</div>;
  if (!isAdmin) {
    return (
      <div className="pt-32 text-center space-y-3">
        <p className="text-muted-foreground">Insufficient privileges.</p>
        <Button variant="ghost" onClick={() => supabase.auth.signOut().then(() => navigate({ to: "/auth" }))}>{t("sign_out")}</Button>
      </div>
    );
  }
  ...
  const save = async (d: Draft) => {
    const payload = {
      image_url: d.image_url!, category: d.category!, price: Number(d.price) || 0,
      in_stock: !!d.in_stock, quantity: Number(d.quantity) || 0,
      size: d.size || null,
      title_en: d.title_en!, title_de: d.title_de!,
      description_en: d.description_en!, description_de: d.description_de!,
      material_en: d.material_en || null, material_de: d.material_de || null,
      color_en: d.color_en || null, color_de: d.color_de || null,
      notes_en: d.notes_en || null, notes_de: d.notes_de || null,
          ...
          <div className="mt-6 grid gap-3">
            {(q.data ?? []).map((p) => (
              <div key={p.id} className="glass rounded-xl p-4 flex items-center gap-4">
                <img src={resolveImage(p.image_url)} alt="" className="h-16 w-16 object-cover rounded-lg bg-secondary" />
      ...
      <Dialog open={!!editing} onOpenChange={(o) => !o && setEditing(null)}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <DialogFooter>
            ...
            <Button onClick={() => save(editing!)} className="bg-gold text-gold-foreground hover:bg-gold/90">{t("save")}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
